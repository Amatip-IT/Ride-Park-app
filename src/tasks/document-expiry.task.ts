...
export class DocumentExpiryTask {
  ...
  async handleDocumentExpiry() {
    ...
    try {
      ...
      this.logger.error(`Document expiry check failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async processDriverDocuments(driver: any, type: 'chauffeur' | 'taxi') {
    const now = new Date();
    ...
    for (const [docField, expiryData] of Object.entries(driver.documentExpiries)) {
      ...
      else if (daysUntilExpiry <= 7 && expiry.renewalReminderLevel !== '7_day') {
      ...
      else if (daysUntilExpiry <= 30 && expiry.renewalReminderLevel !== '30_day' && expiry.renewalReminderLevel !== '7_day') {
  ...
  private async handleExpiredDocument(driver: any, docField: string, type: string) {
    ...
    // Notify driver
    const user = driver.user || (await this.userModel.findById(driver.user).exec());
    if (user) {
      try {
  ...
  private async sendExpiryNotification(driver: any, docField: string, daysRemaining: number, type: string) {
    const user = driver.user || (await this.userModel.findById(driver.user).exec());
    if (!user) return;
    ...
    try {
      ...
      this.logger.warn(`Failed to send ${daysRemaining}-day expiry notification: ${err}`);
    }
  }
}
